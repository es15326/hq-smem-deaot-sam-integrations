...
out_dir = '/cluster/VAST/civalab/results/development_data_2024/pred_dev/'
...
while True:
    ...
    if not imagefile:
        break
    '''with open('/cluster/VAST/civalab/results/development_data_2024/depth_for_test.txt', 'a') as f:
        f.write(f'{imagefile}\n')'''

    img_name = f'{os.path.basename(imagefile).split(".")[0]}.png'
    seq_name = os.path.dirname(imagefile).split('/')[-2]
    image = read_img(imagefile)
    ...
    save_mask(_pred_label, f'{out_dir}/{seq_name}/{img_name}', squeeze_idx=None)
    ...
    pred_resized = cv2.resize(pred_labels, (image.shape[1], image.shape[0]), interpolation=cv2.INTER_NEAREST).astype(np.uint8)


    _masks = transfer_mask(pred_resized, object_num)
    handle.report(_masks)'''
