...
if not os.path.exists(f'{embd_dir}/{seq_name}'):
    os.makedirs(f'{embd_dir}/{seq_name}')
...
fps = 30
out = cv2.VideoWriter(f'{seq_name}_hqsmem_embeddings.mp4', fourcc, fps, (width, height))
...
for img_path in imgs_paths:
    ...
    #predictor.save_image_embedding(f'{embd_dir}/{seq_name}/{os.path.basename(imagefile).split(".")[0]}')
    predictor.load_image_embedding(f'{embd_dir}/{seq_name}/{os.path.basename(imagefile).split(".")[0]}')
    ...
    combined_mask = _pred_label


    object_numbers = len(np.unique(combined_mask)) - 1

    for i in range(1, object_num+1):
        color = colors[i-1]
        image[combined_mask == i] = 0.8 * np.array(color, dtype=np.float32) + 0.2 * image[combined_mask == i].astype(np.float32)
