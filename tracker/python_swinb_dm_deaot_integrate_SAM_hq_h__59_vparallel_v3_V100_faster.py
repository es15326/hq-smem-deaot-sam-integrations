...
def select_best_mask(i, m, masks, scores):
    new_m = m
    max_iou = 0.59
    for j in range(len(masks)):
        ...
        pred_mask = mask[max_index, :, :].cpu().numpy().astype(np.uint8) if len(scores) > 1 else mask.cpu().numpy().astype(np.uint8)
...
embd_dir = '/cluster/VAST/civalab/results/embeddngs/'
...
while True:
    ...
    predictor.save_image_embedding(f'{embd_dir}/embd_V100_faster')

    start_time = time.time()

    with ThreadPoolExecutor() as executor:
        future_load = executor.submit(predictor.load_image_embedding, f'{embd_dir}/embd_V100_faster')
        future_track = executor.submit(
            lambda: F.interpolate(
                torch.tensor(tracker.track(image))[None, None, :, :],
                size=mask_size,
                mode="nearest"
            ).numpy().astype(np.uint8)[0][0]
        )

        labels = future_track.result()
        future_load.result()  # ensure embedding load completed

    input_boxes_list = []
    original_masks = []
    valid_object_ids = []
    predicted_masks = [None] * object_num

    for object_id in range(1, object_num + 1):
        m = (labels == object_id).astype(np.uint8)
        if m.sum() == 0:
            predicted_masks[object_id - 1] = m
            continue

        bbox = extract_bounding_box(m)
        input_boxes_list.append(bbox)
    ...
    if input_boxes_list:
        ...
        with ThreadPoolExecutor() as executor:
            futures = [
                executor.submit(select_best_mask, valid_object_ids[i], original_masks[i], masks_batch[i], scores_batch[i])
                for i in range(len(valid_object_ids))
            ]
            for future in futures:
                object_id, new_m = future.result()
                predicted_masks[object_id - 1] = new_m

    end_time = time.time() - start_time

    with open('/cluster/VAST/civalab/results/development_data_2024/parallel_v3_V100_faster.txt', 'a') as f:
        f.write(f'{imagefile}\t{end_time}')
