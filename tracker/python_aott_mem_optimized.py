...
class AOTTracker(object):
    def __init__(self, cfg, gpu_id, short_term_gap, long_term_gap):
        self.gpu_id = gpu_id
        ...
        self.engine = build_engine(cfg.MODEL_ENGINE,
                                   ...
                                   gpu_id=gpu_id,
                                   short_term_mem_skip=short_term_gap,
                                   long_term_mem_gap=long_term_gap)
    ...
    def track(self, image):
        ...
        _pred_label = F.interpolate(pred_label, size=self.engine.input_size_2d, mode="nearest")
...
# --- CONFIGURATION ---
config = {
    'exp_name': 'default', 'model': 'aott',
    'pretrain_model_path': 'pretrained_models/AOTT_PRE_YTB_DAV.pth',
    'config': 'pre_ytb_dav', 'long_max': 10, 'long_gap': 30, 'short_gap': 2,
    'patch_wised_drop_memories': True, 'patch_max': 20000, 'gpu_id': 0,
}
...
# Get merged-mask for the first frame
merged_mask = np.zeros((image.shape[0], image.shape[1]), dtype=np.int16)
object_num = len(objects)
object_id = 1
for obj_region in objects:
    mask = make_full_size(obj_region, (image.shape[1], image.shape[0]))
    mask = np.where(mask > 0, object_id, 0)
    merged_mask += mask
    object_id += 1

# === RESIZE AND PAD THE *FIRST* FRAME AND MASK ===
# Define a fixed canvas size
TARGET_H, TARGET_W = 480, 856
h, w, _ = image.shape
...
resized_mask = cv2.resize(merged_mask, (new_w, new_h), interpolation=cv2.INTER_NEAREST)

# Create black canvases
padded_image = np.zeros((TARGET_H, TARGET_W, 3), dtype=np.uint8)
padded_mask = np.zeros((TARGET_H, TARGET_W), dtype=np.int16)

# Paste the resized image and mask onto the canvases
...
# Initialize tracker
tracker = AOTTracker(cfg, config["gpu_id"], config["short_gap"], config["long_gap"])
tracker.add_first_frame(padded_image, padded_mask, object_num)
mask_size = (image.shape[0], image.shape[1]) # Keep original mask size for final output
...
while True:
    ...
    # === RESIZE AND PAD SUBSEQUENT FRAMES ===
    h, w, _ = image.shape
    ratio = min(TARGET_W / w, TARGET_H / h)
    new_w, new_h = int(w * ratio), int(h * ratio)
    resized_image = cv2.resize(image, (new_w, new_h), interpolation=cv2.INTER_AREA)
    ...
    # Upscale final mask back to the original full resolution
