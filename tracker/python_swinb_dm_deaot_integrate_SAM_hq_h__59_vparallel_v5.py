...
def get_labels_on_gpu0(image, mask_size, return_dict):
    torch.cuda.set_device(0)
    labels = tracker.track(image)  # tracker must use GPU 0
    labels = F.interpolate(
        torch.tensor(labels)[None, None, :, :].float().cuda(0),
        size=mask_size,
        mode="nearest"
    ).cpu().numpy().astype(np.uint8)[0][0]
    return_dict["labels"] = labels

def set_image_on_gpu1(image, return_dict):
    torch.cuda.set_device(1)
    predictor.set_image(image.cuda(1))  # predictor must be on GPU 1
    return_dict["predictor_set"] = True

# Required on top-level for multiprocessing to work with CUDA
...
while True:
    ...
    p1 = mp.Process(target=get_labels_on_gpu0, args=(image, mask_size, return_dict))
    p2 = mp.Process(target=set_image_on_gpu1, args=(image, return_dict))
    ...
    for object_id in range(1, object_num + 1):
        ...
        input_boxes = torch.tensor(bbox, device="cuda:1")
    ...
    with open('/cluster/VAST/civalab/results/development_data_2024/my_method_timing_dev_vots_v5.txt', 'a') as f:
