...
# Create a Kalman filter instance for each object
filters = [KalmanFilter() for _ in range(object_num)]
...
for img_path in imgs_paths:
    ...
    for object_id in range(1, object_num + 1):
        ...
        if m.sum() == 0:
          bbox = filters[object_id - 1].predict()
          use_kalman = True
        else:
        ...
        if use_kalman:
          new_m = masks[0, 0, :, :].cpu().numpy().astype(np.uint8)
        else:
          new_m, max_iou = m, 0.5
          for mask in masks:
              ...
              if iou > max_iou:
                  ...
                  max_iou = iou
        if new_m.sum() != 0:
          new_bbox = extract_bounding_box(new_m)
          filters[object_id - 1].update(np.array(new_bbox))
        masked_frame = draw_mask(image, new_m)
    out.write(masked_frame)
