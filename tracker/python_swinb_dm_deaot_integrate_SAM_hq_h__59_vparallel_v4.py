...
def get_labels(image, mask_size):
...
def process_object(object_id, labels, image, predictor):
    m = (labels == object_id).astype(np.uint8)
    if m.sum() == 0:
        return m  # return empty mask
...
while True:
    ...
    # Parallel execution of tracker + predictor.set_image
    with ThreadPoolExecutor() as executor:
        future_labels = executor.submit(get_labels, image, mask_size)
        future_set_image = executor.submit(predictor.set_image, image)

        labels = future_labels.result()
        future_set_image.result()

    # Parallel processing of object masks
    with ThreadPoolExecutor() as executor:
        futures = [
            executor.submit(process_object, object_id, labels, image, predictor)
            for object_id in range(1, object_num + 1)
        ]
        predicted_masks = [f.result() for f in futures]
