...
def extract_bounding_boxes(mask):
    ...
    for mask_value in np.unique(mask):
        ...
        bounding_boxes.append([x_min, y_min, x_min+width, y_min+height])
...
fps = 30
out = cv2.VideoWriter(f'{seq_name}_video.mp4', fourcc, fps, (width, height))
...
for img_path in imgs_paths:
    ...
    m = tracker.track(frame)
    pred_mask = F.interpolate(torch.tensor(m)[None, None, :, :], size=mask_size, mode="nearest").numpy().astype(np.uint8)[0][0]
    bboxes = extract_bounding_boxes(pred_mask)
    #####if len(bboxes) > 0:
        #####bbox = bboxes[0]
        #####cv2.rectangle(frame, (bbox[0], bbox[1]), (bbox[0] + bbox[2], bbox[1] + bbox[3]), color, thickness)

    if len(bboxes) > 0:
        ####input_boxes = np.array(bboxes[0])
        
        input_boxes = torch.tensor(bboxes, device=predictor.device)
        transformed_boxes = predictor.transform.apply_boxes_torch(input_boxes, image.shape[:2])
        #####input_box = np.array(bboxes[1])
        #####masks, iou_predictions, _ = predictor.predict(point_coords=None,point_labels=None,box=input_box[None, :],multimask_output=False,)
        ...
        for m in masks:
            pred_mask = (m[0, :, :].cpu().numpy()) * 1
            ####pred_mask = m
            masked_frame = draw_mask(frame, pred_mask)
        out.write(masked_frame)
        #######################################pred_mask = (masks[0, 0, :, :].cpu().numpy()) * 1
        #####pred_mask = masks[0] * 1
        #frame[mask==1] = color
    ############print(masks.shape)
    ...
    ##########################################masked_frame = draw_mask(frame, pred_mask)
    #masked_frame = cv2.cvtColor(pred_mask, cv2.COLOR_RGB2BGR)
    #########################################out.write(masked_frame)'''
...
while True:
    ...
    bboxes = extract_bounding_boxes(m)
    predicted_masks = []
    '''if len(bboxes) > 0:
        ...
        new_m, max_iou = m, 0
        for mask in masks:
            pred_mask = (mask[0, :, :].cpu().numpy().astype(np.uint8)) * 1
            iou = (m * pred_mask + 1e-6).sum() / (np.maximum(m, pred_mask).sum() + 1e-6)
            if iou > max(max_iou, iou_thresh):
              new_m = m
              max_iou = iou
            predicted_masks.append(pred_mask.astype(np.uint8))'''
