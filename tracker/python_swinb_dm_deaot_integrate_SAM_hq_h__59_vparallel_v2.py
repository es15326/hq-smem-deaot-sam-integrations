...
while True:
    ...
    for object_id in range(1, object_num + 1):
        ...
        if m.sum() == 0:
            predicted_masks[object_id - 1] = m  # empty mask
            continue
    ...
    if input_boxes_list:
        ...
        for i, object_id in enumerate(valid_object_ids):
            m = original_masks[i]
            masks = masks_batch[i]
            ...
            for j in range(len(masks)):
                mask = masks[j]
                print(mask.shape, len(masks), scores)
                score = scores[j]
    ...
    with open('/cluster/VAST/civalab/results/VOTS_2024/parallel_v2.txt', 'a') as f:
