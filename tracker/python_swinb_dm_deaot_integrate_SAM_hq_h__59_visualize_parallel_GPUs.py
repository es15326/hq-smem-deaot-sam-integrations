...
out_dir = '/cluster/VAST/civalab/results/development_data_VOTS_2024_prediction/'
...
# Assuming predictor and tracker are initialized as:
# predictor (on GPU:1) and tracker (on GPU:0)

def process_object(object_id, labels, predictor, image, device):
...
for img_path in imgs_paths:
    ...
    # Move image to GPU:0 for tracking, GPU:1 for prediction
    image_gpu0 = torch.tensor(image).to('cuda:0')
    image_gpu1 = image_gpu0.clone().to('cuda:1')


    # Run set_image and track in parallel on two GPUs
    with concurrent.futures.ThreadPoolExecutor() as executor:
        # Predict on GPU:1
        future_predictor = executor.submit(predictor.set_image, image)  # GPU:1

        # Track on GPU:0
        future_tracker = executor.submit(
            lambda: F.interpolate(
                torch.tensor(tracker.track(image), device='cuda:0')[None, None, :, :],
                size=mask_size,
                mode="nearest"
        ...
        future_predictor.result()
        labels = future_tracker.result()

    # Parallel processing of each object mask
    predicted_masks = []
    with concurrent.futures.ThreadPoolExecutor() as executor:
        futures = [
            executor.submit(process_object, object_id, labels, predictor, image_gpu1, predictor.device)
            for object_id in range(1, object_num + 1)
        ]
        for f in concurrent.futures.as_completed(futures):
            predicted_masks.append(f.result())
