...
out_dir = '/cluster/VAST/civalab/results/development_data_VOTS_2024_GT/'
...
number_gt_frames = len(read_trajectory(gt_paths[0]))

for j in range(number_gt_frames):
    initial_mask = np.zeros(first_img.shape[:2], dtype=np.uint8)
    for i, gt_path in enumerate(gt_paths, 1):
        curr_obj = read_trajectory(gt_path)[j]
        if type(curr_obj) == Special or (curr_obj.mask).size == 0:
            continue
    ...
    if np.all(initial_mask == 0):
        continue
    name = f"{j:08d}.png"
    save_mask(initial_mask, f'{out_dir}/{seq_name}/{name}', squeeze_idx=None)
