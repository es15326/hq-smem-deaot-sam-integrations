...
while True:
    ...
    predictor.save_image_embedding(f'{embd_dir}/embd_A100')
    start_time = time.time()
    predictor.load_image_embedding(f'{embd_dir}/embd_A100')

    labels = tracker.track(image)
    labels = F.interpolate(
        torch.tensor(labels)[None, None, :, :],
        size=mask_size,
        mode="nearest"
    ...
    with open('/cluster/VAST/civalab/results/VOTS_2024/parallel_v3_A100.txt', 'a') as f:
